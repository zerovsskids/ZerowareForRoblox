...
run(function()
    local GuiLibrary = shared.GuiLibrary
	local texture_pack = {};
	texture_pack = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		Name = 'TexturePack',
		HoverText = 'Custom pack.',
		Function = function(callback)
			if callback then
...
run(function() 
    local Invisibility = {}
    Invisibility = GuiLibrary.ObjectsThatCanBeSaved.CombatWindow.Api.CreateOptionsButton({
        Name = 'Invisibility',
        Function = function(calling)
            if calling then 
                task.spawn(function()
                    repeat task.wait() until GuiLibrary.ObjectsThatCanBeSaved.AnimationPlayerOptionsButton
                    repeat task.wait() until GuiLibrary.ObjectsThatCanBeSaved.AnimationPlayerSpeedSlider
                    GuiLibrary.ObjectsThatCanBeSaved.AnimationPlayerSpeedSlider.Api.SetValue(9999999999999999999999999)
                    GuiLibrary.ObjectsThatCanBeSaved.AnimationPlayerAnimationTextBox.Api.SetValue('11360825341')
                    if not GuiLibrary.ObjectsThatCanBeSaved.AnimationPlayerOptionsButton.Api.Enabled then
